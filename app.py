...
#db = mysql.connector.connect(host='localhost',user='root',password='lakshmi@1234',database='payroll_management')
...
def search():
    ...
    #         query = ('select emp_id, fullname, designation, gender, email, department from emp_registration where emp_id like %s or fullname like %s designation like %s or gender like %s or email like %s  department like %s')
    #         search_pram = f'%{name}%' #search_pram means search parameteres
    #         cursor.execute(query,[search_pram,search_pram,search_pram,search_pram,search_pram,search_pram])#here we took 4 times search_pram means in sql query we written %s 4 times so we have to match the no of variables to no of parameters.
    #         data = cursor.fetchall()
...
#         email = session.get('email')
#         cursor = db.cursor(buffered=True)
#         cursor.execute('select firstname,lastname,emp_id from emp_registration where email = %s',(email,))
#         result = cursor.fetchone()
#         emp_id = result[2]
#         username = f'{result[0]} {result[1]}'
#         print(username)
#         cursor.close()
#         if request.method == 'POST':
#             date = request.form['date']
#             if not date:
#                 flash('Please Select The Date')
...
#                 db.commit()
#                 cursor.close()
#                 return redirect(url_for('checkin_details',emp_id = emp_id))# Redirect to the dashboard with emp_id
#         return render_template('emp_dashboard.html',username=username)# Render the dashboard if not POST
#     else:
#         flash('You Are Not Logged In')
